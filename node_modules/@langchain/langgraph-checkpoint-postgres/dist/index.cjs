...
* ```
*/
var PostgresSaver = class PostgresSaver extends _langchain_langgraph_checkpoint.BaseCheckpointSaver {
	pool;
	options;
	SQL_STATEMENTS;
	isSetup;
	...
	*/
	static fromConnString(connString, options) {
		return new PostgresSaver(new Pool({ connectionString: connString }), void 0, options);
	}
	/**
	...
	async _dumpWrites(threadId, checkpointNs, checkpointId, taskId, writes) {
		return Promise.all(writes.map(async ([channel, value], idx) => {
			...
			return [
				...
				checkpointId,
				taskId,
				_langchain_langgraph_checkpoint.WRITES_IDX_MAP[channel] ?? idx,
				channel,
				type,
	...
	*/
	async getTuple(config) {
		...
		const [row] = (await this.pool.query(this.SQL_STATEMENTS.SELECT_SQL + where, args)).rows;
		if (row === void 0) return void 0;
		if (row.checkpoint.v < 4 && row.parent_checkpoint_id != null) {
			const [sendsRow] = (await this.pool.query(this.SQL_STATEMENTS.SELECT_PENDING_SENDS_SQL, [thread_id, [row.parent_checkpoint_id]])).rows;
	...
	*/
	...
	async _migratePendingSends(pendingSends, mutableRow) {
		...
		const [enc, blob] = await this.serde.dumpsTyped(await Promise.all(pendingSends.map(([enc, blob]) => this.serde.loadsTyped(textDecoder.decode(enc), blob))));
		row.channel_values ??= [];
		row.channel_values.push([
			textEncoder.encode(_langchain_langgraph_checkpoint.TASKS),
			textEncoder.encode(enc),
			blob
		]);
		row.checkpoint.channel_versions[_langchain_langgraph_checkpoint.TASKS] = Object.keys(mutableRow.checkpoint.channel_versions).length > 0 ? (0, _langchain_langgraph_checkpoint.maxChannelVersion)(...Object.values(mutableRow.checkpoint.channel_versions)) : this.getNextVersion(void 0);
	}
	/**
	...
	*/
	async put(config, checkpoint, metadata, newVersions) {
		...
		const [serializedBlobs, serializedMetadata] = await Promise.all([this._dumpBlobs(thread_id, checkpoint_ns, checkpoint.channel_values, newVersions), this._dumpMetadata(metadata)]);
		const client = await this.pool.connect();
		try {
	...
	*/
	async putWrites(config, writes, taskId) {
		const query = writes.every((w) => w[0] in _langchain_langgraph_checkpoint.WRITES_IDX_MAP) ? this.SQL_STATEMENTS.UPSERT_CHECKPOINT_WRITES_SQL : this.SQL_STATEMENTS.INSERT_CHECKPOINT_WRITES_SQL;
