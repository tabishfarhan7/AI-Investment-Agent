//#region src/store/modules/utils.ts
/**
...
function validateNamespace(namespace) {
	...
	for (const label of namespace) {
		if (typeof label !== "string") throw new Error(`Invalid namespace label '${label}' found in ${namespace}. Namespace labels must be strings, but got ${typeof label}.`);
		if (label.includes(".")) throw new Error(`Invalid namespace label '${label}' found in ${namespace}. Namespace labels cannot contain periods ('.').`);
		if (label === "") throw new Error(`Namespace labels cannot be empty strings. Got ${label} in ${namespace}`);
		if (LIKE_RESERVED_PATTERN.test(label)) throw new Error(`Invalid namespace label '${label}' found in ${namespace}. Namespace labels cannot contain SQL LIKE wildcards ('%', '_') or the backslash escape character ('\\\\'); these would cause search() to match namespaces outside the requested prefix.`);
	}
	if (namespace[0] === "langgraph") throw new Error(`Root label for namespace cannot be "langgraph". Got: ${namespace}`);
}
//#endregion
