...
var SearchOperations = class {
	...
	async executeSearch(client, operation) {
		require_utils.validateNamespace(operation.namespacePrefix);
    ...
    `;
		...
		if (filter && Object.keys(filter).length > 0) {
			const { conditions, newParamIndex } = require_query_builder.QueryBuilder.buildFilterConditions(filter, params, paramIndex);
	...
	async textSearch(namespacePrefix, options = {}) {
		return this.core.withClient(async (client) => {
			require_utils.validateNamespace(namespacePrefix);
...
//# sourceMappingURL=search-operations.cjs.map