...
const require_database_core = require("./modules/database-core.cjs");
const require_vector_operations = require("./modules/vector-operations.cjs");
const require_crud_operations = require("./modules/crud-operations.cjs");
const require_search_operations = require("./modules/search-operations.cjs");
const require_ttl_manager = require("./modules/ttl-manager.cjs");
const require_sql = require("./sql.cjs");
const require_store_migrations = require("./store-migrations.cjs");
...
*/
var PostgresStore = class PostgresStore extends _langchain_langgraph_checkpoint.BaseStore {
	core;
	vectorOps;
	crudOps;
	searchOps;
	...
	constructor(config) {
		super();
		this.core = new require_database_core.DatabaseCore(typeof config.connectionOptions === "string" ? new Pool({ connectionString: config.connectionOptions }) : new Pool(config.connectionOptions), config.schema || "public", config.ttl, config.index, config.textSearchLanguage);
		this.vectorOps = new require_vector_operations.VectorOperations(this.core);
		this.crudOps = new require_crud_operations.CrudOperations(this.core, this.vectorOps);
		this.searchOps = new require_search_operations.SearchOperations(this.core, this.vectorOps);
		this.ttlManager = new require_ttl_manager.TTLManager(this.core);
	...
	*/
	async runStoreMigrations() {
		...
		const STORE_TABLES = require_sql.getStoreTablesWithSchema(this.core.schema);
		try {
			await client.query(`CREATE SCHEMA IF NOT EXISTS "${this.core.schema}"`);
			let version = -1;
			const migrations = require_store_migrations.getStoreMigrations({
	...
	*/
    ...
    `;
		...
		let namespaces = (await client.query(sqlQuery, params)).rows.map((row) => row.namespace_path.split(":"));
