  ...
  `);
	if (indexConfig) {
      ...
      FOREIGN KEY (namespace_path, key) REFERENCES ${STORE_TABLES.store}(namespace_path, key) ON DELETE CASCADE
		...
		const metricsToIndex = createAllMetricIndexes ? [
			"cosine",
			"l2",
			"inner_product"
		] : [distanceMetric];
		for (const metric of metricsToIndex) {
			let metricSuffix;
			if (metric === "cosine") metricSuffix = "cosine";
			else if (metric === "l2") metricSuffix = "l2";
			else metricSuffix = "ip";
          ...
          WITH (lists = ${lists});`;
			} else throw new Error(`Unsupported vector index type: ${indexType}`);
...
//#endregion
exports.getStoreMigrations = getStoreMigrations;

//# sourceMappingURL=store-migrations.cjs.map