...
var PostgresStore = class PostgresStore extends BaseStore {
	...
	constructor(config) {
		super();
		this.core = new DatabaseCore(typeof config.connectionOptions === "string" ? new Pool({ connectionString: config.connectionOptions }) : new Pool(config.connectionOptions), config.schema || "public", config.ttl, config.index, config.textSearchLanguage);
	...
	*/
	async runStoreMigrations() {
		...
		try {
			...
			let version = -1;
			const migrations = getStoreMigrations({
